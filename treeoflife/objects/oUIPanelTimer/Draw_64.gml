...
if (show) {
	scr_draw_set(1, c_white, function() {
		...
		/// Bar
		if (time > 0) draw_sprite_stretched_ext(sPlayer, 0, window_get_width() - 218, y-17, 200 * ((time) / startTime) , 35, 195513, image_alpha);
		else draw_sprite_stretched_ext(sPlayer, 0, window_get_width() - 218, 23, 200, 35, 195513, (y/ystart));
		...
		if(time > 0 && time != startTime && string_length(postText) == 0) 
			draw_text_ext_transformed(window_get_width() - 118, 11, string(time), 0, 400,((y*sign(y))/ystart),1,0);
